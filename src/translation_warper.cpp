...
cv::Mat TranslationWarper::matrix(const double* params) const {
  ...
  return cv::Mat(cv::Mat_<double>(2, 3) << 1, 0, x,
                                           0, 1, y);
}

bool TranslationWarper::isValid(const cv::Size& image_size,
                                const double* params,
                                int radius) const {
  double x = params[0];
  double y = params[1];
  ...
  int width = image_size_.width - 2. * radius;
  int height = image_size_.height - 2. * radius;
